...
var _ = BeforeSuite(func() {
	...
	testEnv = &envtest.Environment{
		CRDDirectoryPaths:     []string{filepath.Join("..", "..", "config", "crd", "bases"), filepath.Join("testdata", "crds")},
		ErrorIfCRDPathMissing: true,

		// Controlplane output can be useful to troubleshoot
		// environment issues
	...
	err = synv1alpha1.AddToScheme(scheme.Scheme)
	Expect(err).NotTo(HaveOccurred())

	// Add the necessary Flux resources to the scheme
	err = fluxsource.AddToScheme(scheme.Scheme)
	Expect(err).NotTo(HaveOccurred())
	err = fluxkustomize.AddToScheme(scheme.Scheme)
	Expect(err).NotTo(HaveOccurred())
	...
	controller := &SupervisedGitReconciler{
